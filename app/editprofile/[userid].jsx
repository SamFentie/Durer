...
const params = useLocalSearchParams();
  ...
  const userProfile=sampleProfileData.filter((profile)=>profile.id===userId)
  const [name, setName] = useState(userProfile[0].companyName);
  const [phoneNumber, setPhoneNumber] = useState(userProfile[0].contacts.phone);
  const [bio, setBio] = useState(userProfile[0].bio);
  const [gender, setGender] = useState(userProfile[0].gender);
  const [dateOfBirth, setDateOfBirth] = useState(userProfile[0].dob);
  const [country, setCountry] = useState(userProfile[0].address.country);
